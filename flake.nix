{
  ...
  outputs = { self, nixpkgs, nixpkgs_unstable, home-manager, deploy-rs, david-config }:
  let
  ...
  in {
    ...
    deploy.nodes.Oracle = {
      ...
      user = "root";
      sshUser = "root";
      profiles.system.path = deploy-rs.lib.x86_64-linux.activate.nixos self.nixosConfigurations.Oracle;
    };
